...
namespace Blade.MG.UI
{
    public partial class UIManager //: GameEntity
    {
        ...
        private async Task HandleKeyboardInputAsync(UIWindow eventLockedWindow, UIComponent eventLockedControl, bool propagateEvents, GameTime gameTime)
        {
            ...
            Keys[] lastPressedKeys = InputManager.LastKeyboardState.GetPressedKeys();
            for (int i = 0; i < lastPressedKeys.Count(); i++)
            {
                ...
                {
                    var uiEvent = new UIKeyEvent(InputManager.KeyboardState) { Key = key, KeyChar = KeyboardMapping.Keyboard.GetChar(key, InputManager.KeyboardState) };
            ...
            for (int i = 0; i < pressedKeys.Count(); i++)
            {
                ...
                if (!InputManager.LastKeyboardState.IsKeyDown(key))
                {
