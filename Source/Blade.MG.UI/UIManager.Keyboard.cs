        ...
        private async Task HandleKeyboardInputAsync(UIWindow eventLockedWindow, UIComponent eventLockedControl, bool propagateEvents, GameTime gameTime)
        {
            ...
            for (int i = 0; i < pressedKeys.Count(); i++)
            {
                ...
                if (!InputManager.Keyboard.LastKeyboardState.IsKeyDown(key))
                {
                    ...
                    // Enter/Space activates whichever control currently has focus - calls
                    // ActivateAsync directly on it, same technique as UIManager.GamePad.cs's
                    // A-button handling (see its comment for why this bypasses hit-test dispatch
                    // entirely rather than synthesizing a fake click). Gated on !uiEvent.Handled
